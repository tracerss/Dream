...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 分配基金 <span
        class="c-gray en">&gt;</span> 分配名单 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
<div class="page-container">
    ...
    <%-- <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l">&nbsp;&nbsp;&nbsp;<a href="/index/add"
                                                                                  class="btn btn-success radius"><i
             class="Hui-iconfont">&#xe6e2;</i> 添加</a></span>--%> <span class="r">共有数据：<strong>1</strong> 条</span></div>
<%--<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datasearch()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 查询</a></span>  </div>--%>
<div class="mt-20">
    ...
    <table class="table table-border table-bordered table-bg table-sort">
        ...
        <c:forEach items="${userList}" var="t_staff">
            <tr>
                <td>${t_staff.staffId}</td>
                    <%--<td>${user.firstName} ${user.lastName}</td>--%>
                <td>${t_staff.name}</td>
                <td>${t_staff.department}</td>
                <td>${t_staff.postion}</td>
                    <%--                    <td>${t_staff.entrytime}</td>
                                        <td>${t_staff.createtime}</td>
                                        <td>${t_staff.updatetime}</td>--%>


                <td>
                    <a href="index/show/${t_staff.staffId}" type="button" class="btn btn-sm btn-success">详情</a>
                    <a href="/index/update/${t_staff.staffId}" type="button" class="btn btn-sm btn-warning">修改</a>
                    <a href="/index/delete/${t_staff.staffId}" type="button" class="btn btn-sm btn-danger">删除</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
    ...
    <%--            <c:forEach items="${userList}" var="user">
                <tr>
                    ...
                    <td>${user.password}</td>
                    <td>
                        <a href="/index/show/${user.id}" type="button" class="btn btn-sm btn-success">详情</a>
                        <a href="/index/update/${user.id}" type="button" class="btn btn-sm btn-warning">修改</a>
                        <a href="/index/delete/${user.id}" type="button" class="btn btn-sm btn-danger">删除</a>
                    </td>
                </tr>
                </c:forEach>
            </table>--%>
